...
static inline quint32 _swapu32(quint32 n)
{
	return (    ((n & 0xffU) << 24) |
				((n & 0xff00U) << 8) |
				((n & 0xff0000U) >> 8) |
...
static quint32 _bf_p[BLOWFISHPSIZE] = {
...
/**
 * Configure the cipher with the provided key.
 *
 * @param key The key value.
 * @return 0 on success, -1 on failure
 */
int Blowfish::setup(const QByteArray &key)
{
    int result = setup(&encryptParam, (quint8*)key.data(), key.size() * 8);
    if (result == 0) {
        result = setup(&decryptParam, (quint8*)key.data(), key.size() * 8);
    }
    return result;
}

/**
 * Encrypt the provided data using the specified CBC initialization vector.
 * Thetotal number of bytes in the input data must be a multiple of the
 * Blowfishblock size; if padding is necessary, it must be performed before
 * the data is passed in here.
 *
 * @param data The data to be encrypted
 * @param iv The initialization vector to use for CBC block mode
 * @return The encrypted data
 */
QByteArray Blowfish::encrypt(const QByteArray &data, const QByteArray &iv)
{
    if (iv.size() != (int)Blowfish::blockSize) {
        return QByteArray();
    }
    if (setIV(&encryptParam, (quint8*)iv.data()) != 0) {
        return QByteArray();
    }
    if (data.size() % Blowfish::blockSize != 0) {
        return QByteArray();
    }
    int blockCount = data.size() / (Blowfish::blockSize);
    QByteArray result;
    result.resize(data.size());
    if (blockEncryptCBC((quint32*)result.data(), (quint32*)data.data(), blockCount) != 0) {
        return QByteArray();
    }
    return result;
}
...
/**
 * Decrypt the provided data using the specified CBC initialization vector.
 *
 * @param data The data to be decrypted
 * @param iv The initialization vector to use for CBC block mode
 * @return The decrypted data
 */
QByteArray Blowfish::decrypt(const QByteArray &data, const QByteArray &iv)
{
    if (iv.size() != (int)Blowfish::blockSize) {
        return QByteArray();
    }
    if (setIV(&decryptParam, (quint8*)iv.data()) != 0) {
        return QByteArray();
    }
    if (data.size() % Blowfish::blockSize != 0) {
        return QByteArray();
    ...
    QByteArray result;
    result.resize(data.size());
    if (blockDecryptCBC((quint32*)result.data(), (quint32*)data.data(), blockCount) != 0) {
        return QByteArray();
    }
    return result;
}
...
/**
 * This function encrypts a number of data blocks in Cipher Block Chaining
 * mode.
 * 
...
/**
 * This function decrypts a number of data blocks in Cipher Block Chaining
 * mode.
 * 
...
/**
 * Perform the cipher's key expansion.
 *
 ...
 * @return 0 on success, -1 on failure.
 */
int Blowfish::setup(blowfishParam *bp, const quint8* key, size_t keybits)
{
	if (((keybits & 7) == 0) && (keybits >= 32) && (keybits <= 448))
	{
		...
		memcpy(s, _bf_s, 1024 * sizeof(quint32));
...
/**
 * Set the Initialization Vector.  Note: this function is only useful in
 * block chaining or feedback modes.
 *
 * @param bp The cipher's parameter block.
 * @param iv The initialization vector; may be null.
 * @return 0 on success.
 */
int Blowfish::setIV(blowfishParam *bp, const quint8* iv)
{
	if (iv)
