...
QDatePicker::QDatePicker(QDate *inDate): QDialog(0, 0, TRUE)
{
    ...
    vbox->addWidget(calendarTable, 0, Qt::AlignHCenter);
    ...
    : QGridView(parent)
{
    ...
    setMinimumWidth((7 * w) + 4);
    setMaximumWidth((7 * w) + 4);
    setMinimumHeight((6 * h) + 4);
    setMaximumHeight((6 * h) + 4);
}
//
...
//
    ...
    int nSelRow = rowAt(clickedPos.y());
    int nSelCol = columnAt(clickedPos.x());
//
