  ...
  const char *name, WFlags f) : QDialog(parent, name, TRUE, f), db(subject), importDone(FALSE)
{
    ...
    selector = new SlFileSelector("", mimeType, this, "importselector");
    vbox->addWidget(selector);
    connect(selector, SIGNAL(fileSelected(const QFileInfo &)), this,
            SLOT(import(const QFileInfo &)));
    Config conf("portabase");
    conf.setGroup("Files");
    if (conf.readEntry("View", "Icon") == "Icon") {
        selector->setIconView();
    }
    else {
        selector->setListView();
    }
    showMaximized();
}
...
void ImportDialog::import(const QFileInfo &f)
{
    if (import(f.absFilePath())) {
        accept();
    }
    else {
        reject();
...
int ImportDialog::exec()
{
    ...
    if (result && !importDone) {
        ...
        const QFileInfo *f = selector->selected();
        if (!f) {
